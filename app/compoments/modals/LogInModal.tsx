...
import { closeLoginModal, openForgotModal, openSignupModal } from "@/redux/slices/modalSlice"
  ...
  const isOpen = useSelector((state:RootState) => 
   ...
   async function logInByGoogle(){
    ...
    async function logInAsGuest(){
      try {
        const userCredential = await signInWithEmailAndPassword(auth, "guest12345000@gmail.com","12345678")
        if(userCredential){
          router.push("/dashboard")
        } else {
          throw new Error('Firebase: Error(auth/user-not-found)')
    ...
    >
      ...
      bg-white">
        ...
        <div className="flex flex-col">
          <div
          className="py-[12px] space-y-[5px] gap-[12px] w-full"
          >
          ...
          <div
          className="py-[12px] space-y-[5px] gap-[12px] w-full mb-[20px]"
          >
            <label className="text-[13px] font-medium text-[#667085]">
              Password
            </label>
          ...
          <span className="text-[13px] text-[#4f46e5] mb-[12px] cursor-pointer text-right"
          onClick={() => {
          ...
          items-center 
          " onClick={() => handleLogIn()}
          >{loading ? <CircularProgress className="w-[20px] h-[20px] animate-loading-spinner"/> :"Log In"}</button>
        </div>

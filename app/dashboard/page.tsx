...
function Dashboard() {
  ...
  const [selectedRef] = useEmblaCarousel({
    loop: true,
    align: "start",
    slidesToScroll: 1,
    containScroll: "trimSnaps"
  })
  const [topRef] = useEmblaCarousel({
    loop: true,
    align: "start",
    slidesToScroll: 1,
    containScroll: "trimSnaps"
      ...
      <div className={`w-full ml-0 flex flex-col md:ml-[230px] md:w-[calc(100%-230px)]`}>
        <div className="w-full h-[80px] border-b-[1px] border-solid border-[#f1f3f4]">
          <div className="gap-[20px] relative w-full h-full max-w-[1400px] flex items-center justify-between py-0 px-[32px] my-0 mx-auto">
            ...
            <div className="flex flex-col absolute bg-white w-full max-w-[440px] overflow-y-auto max-h-[440px]
        ...
        <div className="pt-[40px]">
          <div className="flex flex-col items-start w-full h-full max-w-[1400px] justify-between py-0 px-[32px] my-0 mx-auto">
            ...
            <div className="overflow-hidden w-full cursor-grab active:cursor-grabbing" ref={selectedRef}>
              <div className="flex gap-[20px] py-[16px] pl-[20px]">
                {!loading ? moviesList.map((movie) => (
                  <Link
                    key={movie.id}
                    className="flex-[0_0_180px] min-w-[180px]"
                    href={`/movies/${movie.id}`}
                  >
                    <MovieCard
                      image={movie.imageLink}
                      premium={movie.subscriptionRequired && !plan ? "Premium" : undefined}
                      title={movie.title}
                ...
                )) : new Array(5).fill(0).map((_, index) => <MovieCardSkeleton key={index} />)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-[40px_0_88px]">
          <div className="w-full h-full max-w-[1400px] flex justify-between py-0 px-[32px] my-0 mx-auto flex-col items-start">
            ...
            <div className="overflow-hidden w-full cursor-grab active:cursor-grabbing" ref={topRef}>
              <div className="flex gap-[20px] py-[16px] pl-[20px]">
